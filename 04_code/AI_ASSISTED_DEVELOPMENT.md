...
AI 根据冻结的 `03_model/tournament_protocol.json`、D-005、D-007、D-011 和 `3coding-visual` 约束，辅助实现：冻结门禁、L0 数学/输入检查、q1--q5 确定性 baseline、十个挑战者、独立搜索接受评分、统一预算/停止状态、稀疏因子持久化、独立复算、可分片/续跑 runner、真实 L3/L4 变体 adapter、批次谱系与严格失败关闭聚合器。AI 只运行非正式 smoke/readiness，未运行正式 1442-run L2，未选择最终胜者，未改写冻结协议。
