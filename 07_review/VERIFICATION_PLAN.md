...
本计划定义未来验收动作，不是验收报告。当前总状态为 `BLOCKED`：工作流仍处于 `INIT`，官方规则与输入来源未核验，模型结果未冻结，论文尚未生成。任何强制项为 `FAIL`、`BLOCKED`、`NOT_RUN`、`UNKNOWN` 或阻断性 `WARN` 时，总状态必须为 `FAIL`，不得声称“验收通过”“提交就绪”。
...
退出码为 1，状态为 `FAIL`。阻断项包括：官方规则、模板、AI 政策、提交要求和输入来源未核验；problem、tournament protocol、model results 冻结缺失；检索、协议、指标与对擂结果未通过或无正式记录。因此当前不运行论文编译、逐页检查或最终硬验收，也不创建伪造的 PASS 记录。
...
| 2 | 官方规则与输入来源 | 规则/输入核验记录 | 当届模板、AI、提交规则和正式输入来源均 PASS | `BLOCKED` |
...
- 竞赛年份、官方规则来源、要求的文件名、格式、大小限制和匿名要求；
...
官方要求 RAR 时必须生成并回读真实 RAR；工具缺失时只能标记候选包。人工最终上传、平台侧 MD5/哈希确认和官网回执由队长完成，不属于机器 PASS 的替代物。
